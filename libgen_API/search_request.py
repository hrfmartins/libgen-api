...
def ads_cleanup(data):
    new = []
    for el in data:
        if not re.match(".*ad.*", el["ID"]):
            new.append(el)
    return new
...
class SearchRequest:
    ...
    def aggregate_request_data(self):
        ...
        # Determines whether the link url (for the mirror) or link text (for the title) should be preserved. Both the
        # book title and mirror links have a "title" attribute, but only the mirror links have it filled. (title vs
        # title="libgen.io")
        raw_data = [
            [
        ...
        output_data = ads_cleanup(output_data)
        return output_data
